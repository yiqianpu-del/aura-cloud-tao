...
export default function FengShuiDetail({ params }: { params: { slug: string } }) {
  ...
  if (!service) notFound();
  if (service.slug === 'fengshui-consultation') {
    return (
      <div className="section"><article className="container max-w-4xl">
        ...
        {service.pricing.options && (<section className="mb-12"><h2 className="text-2xl font-bold mb-6">Pricing</h2><div className="grid md:grid-cols-3 gap-4">{service.pricing.options.map((opt, i) => (<div key={i} className="card text-center"><h3 className="font-bold text-sm mb-2">{opt.name}</h3><p className="text-2xl font-bold text-accent">${opt.price}</p></div>))}</div></section>)}
        ...
        <div className="text-center pt-8 border-t border-gray-200"><a href={siteConfig.whatsappLink} target="_blank" className="btn btn-primary btn-lg">Book on WhatsApp</a></div>
      </article></div>
    );
  }
  notFound();
