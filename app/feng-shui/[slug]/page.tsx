...
const fsServices = services.filter(s => s.category === 'fengshui');

export function generateStaticParams() {
  return fsServices.map((s) => ({ slug: s.slug }));
}

export function generateMetadata({ params }: { params: { slug: string } }) {
  const s = fsServices.find(s => s.slug === params.slug);
  if (!s) return {};
  return { title: `${s.title} | ${siteConfig.name}`, description: s.tagline };
}

export default function FengShuiDetail({ params }: { params: { slug: string } }) {
  const service = fsServices.find((s) => s.slug === params.slug);
  if (!service) notFound();
  ...
  return (
    <div className="section"><article className="container max-w-4xl">
      <nav className="text-sm text-gray-500 mb-8"><Link href="/" className="hover:text-accent">Home</Link> / <Link href="/feng-shui" className="hover:text-accent">Feng Shui</Link> / <span className="text-ink">{service.title}</span></nav>
      ...
      <div className="grid md:grid-cols-2 gap-8 mb-16">
        ...
        <div>
          ...
          <a href={siteConfig.whatsappLink} target="_blank" className="btn btn-gold btn-lg w-full text-center">Book a Consultation — WhatsApp</a>
        </div>
      </div>
      ...
      {service.pricing.options && (<section className="mb-12"><h2 className="text-2xl font-bold mb-6">Pricing</h2><div className="grid md:grid-cols-3 gap-4">{service.pricing.options.map((opt, i) => (<div key={i} className="card text-center"><h3 className="font-bold text-sm mb-2">{opt.name}</h3><p className="text-2xl font-bold text-accent">${opt.price}</p><a href={siteConfig.whatsappLink} target="_blank" className="btn btn-gold btn-sm mt-2">Book</a></div>))}</div></section>)}
      ...
      {/* Related Products */}
      {relatedProds.length > 0 && (<section className="mb-12"><h2 className="text-2xl font-bold mb-6">Complementary Products</h2><div className="grid md:grid-cols-2 gap-4">{relatedProds.map((p) => (<Link key={p.id} href={`/shop/${p.slug}`} className="card hover:border-gold/50 group flex gap-4 items-center"><div className="w-20 h-20 shrink-0 bg-cream rounded overflow-hidden"><Image src={p.image} alt={p.name} width={80} height={80} className="w-full h-full object-contain p-2" /></div><div><h3 className="font-bold group-hover:text-accent">{p.name}</h3><p className="text-sm text-gray-500">${p.price}</p></div></Link>))}</div></section>)}

      {/* Recommended Journey */}
      <section className="mb-12 p-8 bg-cream rounded-sm border border-gray-200">
        <h2 className="text-2xl font-bold mb-4">Deepen Your Practice</h2>
        <div className="grid md:grid-cols-3 gap-4">
          <Link href="/divination" className="card hover:border-gold/50 group text-center"><span className="text-2xl mb-2 block">🗡</span><h3 className="font-bold group-hover:text-accent">Qi Men Reading</h3><p className="text-xs text-gray-500">Understand the energy of your space</p></Link>
          <Link href="/services" className="card hover:border-gold/50 group text-center"><span className="text-2xl mb-2 block">🪔</span><h3 className="font-bold group-hover:text-accent">Book a Ritual</h3><p className="text-xs text-gray-500">Support your space with a ceremony</p></Link>
          <Link href="/shop/qi-aligner" className="card hover:border-gold/50 group text-center"><span className="text-2xl mb-2 block">🎴</span><h3 className="font-bold group-hover:text-accent">Qi Aligner Device</h3><p className="text-xs text-gray-500">Maintain the energy long-term</p></Link>
        </div>
      </section>
      ...
      <div className="text-center pt-8 border-t border-gray-200 space-y-3">
        ...
        <p className="text-sm text-gray-500"><Link href="/feng-shui" className="text-accent hover:underline">← Back to Feng Shui</Link></p>
      </div>
    </article></div>
  );
}
