...
export function generateStaticParams() {
  return ritualServices.map((s) => ({ slug: s.slug }));
}

export function generateMetadata({ params }: { params: { slug: string } }) {
  const service = ritualServices.find((s) => s.slug === params.slug);
  if (!service) return {};
  return { title: `${service.title} | ${siteConfig.name}`, description: service.tagline };
}
    ...
    <div className="section">
      <article className="container max-w-4xl">
        <nav className="text-sm text-gray-500 mb-8">
          <Link href="/" className="hover:text-accent">Home</Link> / <Link href="/services" className="hover:text-accent">Rituals</Link> / <span className="text-ink">{service.title}</span>
        </nav>

        {/* Hero */}
        <div className="grid md:grid-cols-2 gap-8 mb-16">
          <div className="aspect-square bg-cream rounded-sm overflow-hidden border border-gray-200">
            <Image src={service.image} alt={service.title} width={400} height={400} className="w-full h-full object-contain p-8" />
          </div>
          <div>
            ...
            <p className="text-xs text-gray-500 mb-6">{service.delivery}</p>
            <a href={siteConfig.whatsappLink} target="_blank" className="btn btn-gold btn-lg w-full text-center mb-2">Book This Ritual — WhatsApp</a>
          </div>
        </div>

        {/* Who Needs It */}
        <section className="mb-12">
          <h2 className="text-2xl font-bold mb-6">Who Needs This</h2>
          <ul className="grid md:grid-cols-2 gap-3">
            {service.whoNeedsIt.map((item, i) => (
              <li key={i} className="flex items-start gap-2 text-sm text-gray-600"><span className="text-gold mt-0.5 shrink-0">✦</span> {item}</li>
            ))}
          </ul>
        </section>
        ...
        {/* Process */}
        <section className="mb-12">
          <h2 className="text-2xl font-bold mb-6">The Ceremony Process</h2>
          <div className="space-y-6">
            {service.process.map((step) => (
              <div key={step.step} className="flex gap-4 items-start">
                <span className="w-8 h-8 rounded-full bg-gold text-ink font-bold flex items-center justify-center shrink-0 text-sm">{step.step}</span>
                <div><h3 className="font-semibold">{step.title}</h3><p className="text-sm text-gray-600">{step.description}</p></div>
              </div>
            ))}
          </div>
        </section>
