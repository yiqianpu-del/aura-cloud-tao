    ...
    <div className="section">
      <article className="container max-w-4xl">
        ...
        <section className="mb-12">
          ...
          <ul className="grid md:grid-cols-2 gap-3">{service.whoNeedsIt.map((item, i) => (<li key={i} className="flex items-start gap-2 text-sm text-gray-600"><span className="text-gold mt-0.5 shrink-0">✦</span> {item}</li>))}</ul>
        </section>

        <section className="mb-12">
          <h2 className="text-2xl font-bold mb-6">Process</h2>
          <div className="space-y-6">{service.process.map((step) => (<div key={step.step} className="flex gap-4 items-start"><span className="w-8 h-8 rounded-full bg-gold text-ink font-bold flex items-center justify-center shrink-0 text-sm">{step.step}</span><div><h3 className="font-semibold">{step.title}</h3><p className="text-sm text-gray-600">{step.description}</p></div></div>))}</div>
        </section>

        {service.pricing.options && service.pricing.options.length > 0 && (
          <section className="mb-12"><h2 className="text-2xl font-bold mb-6">Pricing</h2><div className="grid md:grid-cols-3 gap-4">{service.pricing.options.map((opt, i) => (<div key={i} className="card text-center"><h3 className="font-bold text-sm mb-2">{opt.name}</h3><p className="text-2xl font-bold text-accent">${opt.price}</p></div>))}</div></section>
        )}
